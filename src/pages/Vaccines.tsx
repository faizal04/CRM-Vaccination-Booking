...
export default function Vaccines() {
  ...
  async function toggleActive(v: Vaccine) {
    ...
    setVaccines((prev) =>
      prev.map((x) => (x.id === v.id ? { ...x, is_active: next } : x)),
    );
  }
    ...
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <div className="flex items-end justify-between mb-6">
        <div>
          ...
          <p className="text-body-md text-on-surface-variant">
            Manage the vaccine catalogue — names, descriptions, and prices shown
            to patients during booking.
          </p>
        </div>
        <button
          onClick={openAdd}
      ...
      <div className="bg-white rounded-xl shadow-sm p-4 flex gap-4 items-center border border-slate-100 mb-4">
        ...
        <span className="text-xs text-slate-400 font-medium">
          {filtered.length} vaccine{filtered.length !== 1 ? "s" : ""}
        </span>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map((v) => (
                <tr
                  key={v.id}
                  className={`text-sm transition-colors ${v.is_active ? "hover:bg-slate-50" : "bg-slate-50/60 opacity-60 hover:opacity-80"}`}
                >
                  {/* Name */}
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`text-xs font-semibold ${v.is_active ? "text-emerald-700" : "text-slate-400"}`}
                      >
              ...
              <div className="flex items-center justify-between py-1">
                <div>
                  ...
                  <p className="text-xs text-slate-400">
                    Inactive vaccines won't appear in the booking form
                  </p>
                </div>
                <button
                  type="button"
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center">
            ...
            <p className="text-slate-500 text-sm mb-6">
              <strong>{vaccines.find((v) => v.id === deleteId)?.name}</strong>{" "}
              will be permanently removed. Existing bookings won't be affected.
            </p>
